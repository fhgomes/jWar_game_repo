 ...
 * @author Fernando
 */
public class FrameChat extends javax.swing.JFrame implements IChatGUIXml, SalaSelectionListener{

	private HashMap<String, Integer> salas = new HashMap<String, Integer>();
    ...
    private void tfMensagemActionPerformed(java.awt.event.ActionEvent evt) {
    	try {
			ClientSession.getSession().getChat().
					escrever(ClientSession.getSession().getNomeChat(), tfMensagem.getText(), ClientSession.getSession().getJogador());
			tfMensagem.setText("");
    	} catch (Exception e) {
