...
public class FrameLogin extends JDialog{
    ...
    private void actionBtServer(String s) {
    	if(validaCampos()) {
	    	ServidorCommand c = new ServidorCommand();
	    	FrameChat fc = new FrameChat();
			try {				
				RepositorioFrames.getRepositorio().show("Chat", fc, false);
				c.executeCommand(s, tfEmail.getText(), tfSenha.getText(), fc);	
				fc.setVisible(true);
				this.dispose();
	...
	protected void actionBtCadastrar(ActionEvent evt) {
		RepositorioFrames.getRepositorio().show("Cadastro", new FrameCadastroJogador());
	}
